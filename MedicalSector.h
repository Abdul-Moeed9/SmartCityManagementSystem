...
public:
	...
	void displayEmergencyBeds() {
		cout << "\n Emergency Bed Availability" << endl;
		cout << endl;
		...
		cout << "\n Hospitals ranked by bed availability: " << endl;
		int rank = 1;
	...
	void searchMedicineByName(string name) {
		if (!InputValidator::isValidString(name)) {
			cout << "\n Error: Invalid name of medicine" << endl;
			return;
		}
	...
	void displayAllPharmacies() {
		...
		cout << "\n Total Pharmacies: " << keycount << endl;
		cout << "-----------------------------------" << endl;
		delete[]keys;
	}
	...
	void findHospitalsBySpecialization(string specialization) {
		if (!InputValidator::isValidString(specialization)) {
			cout << "\n Error: Invalid Specialization" << endl;
			return;
		}
		cout << "\n Hospitals With " << specialization << endl;
		cout << "--------------------------------------" << endl;
		...
		if (found == 0) {
			cout << "\n No Hospitals Found with this specialization" << endl;
		}
		else {
			cout << "\n Total Hospitals Found: " << found << endl;
		}
		cout << "-----------------------------------------" << endl;
		delete[]keys;
	}

	//Find Nearest Hospital dijkstra
	void findNearestHospital(double lat, double lon) {
		...
		cout << "\n      NEAREST HOSPITAL FINDER (Using Dijkstra)" << endl;
		cout << "========================================" << endl;
		...
		string userNodeID = "USER_TEMP";
		medicalGraph->addNode(userNodeID, "Your Location", lat, lon);
		
		// Connect user to ALL hospitals directly (no hospital-to-hospital connections)
		for (int i = 0; i < keyCount; i++) {
			Hospital* hospital = (Hospital*)hospitalTable.search(keys[i]);
			if (hospital) {
				if (!medicalGraph->nodeExists(hospital->id)) {
					medicalGraph->addNode(hospital->id, hospital->name,
						hospital->latitude, hospital->longitude);
				}
				double dist = MathUtils::calculateDistance(lat, lon, hospital->latitude, hospital->longitude);
				if (dist >= 0 && dist < 1000.0) {
					medicalGraph->addEdge(userNodeID, hospital->id, dist);
				}
			}
		}

		// Calculate distances using Dijkstra (limit to 10 hospitals to save memory)
		...
		for (int i = 0; i < keyCount; i++) {
			...
			if (hospital) {
				double totalDist = 0;
				string* path = medicalGraph->dijkstra(userNodeID, hospital->id, totalDist);

				if (totalDist > 0 && totalDist < 999999.0) {
					results[resultCount].hosp = hospital;
					results[resultCount].dist = totalDist;
					resultCount++;
				}

				// IMMEDIATELY clean up path
				if (path) {
					delete[] path;
					path = nullptr;
				}
		...
		for (int i = 0; i < displayCount; i++) {
			...
			cout << "   Route: Direct path from your location" << endl;
			cout << endl;
		}

		delete[] results;
		delete[] keys;
		medicalGraph->removeNode(userNodeID);
		if (resultCount == 0) {
			cout << "\nNo reachable hospitals found!" << endl;
		}
	...
	void findDoctorsBySpecialization(string specialization) {
		...
		cout << "\n Doctors with Specialization: " << specialization << endl;
		cout << "--------------------------------------" << endl;
		...
		cout << "\n Total doctors found: " << count - 1 << endl;
		cout << "--------------------------------------" << endl;
	}
	void viewAllPatients() {
		cout << "\n All Patients in System" << endl;
		cout << "-----------------------" << endl;
		int keyCount = 0;
	...
	void displayPatientRecord(string cnic) {
		...
		if (!patient) {
			cout << "\n Patient record not found for CNIC: " << cnic << endl;
			return;
		}

		cout << "\n Patient Medical Record" << endl;
		cout << "-----------------------" << endl;
		cout << "CNIC: " << patient->cnic << endl;
	...
	void addMedicineToPharmacy() {
		...
		formulaList->insert((void*)medicine);
		cout << "\n Medicine added successfully to the pharmacy" << endl;
	}
