...
public:
	...
	void displayAllJourneys() {
		...
		for (int i = 0; i < keycount; i++) {
			...
			if (journey) {
				cout << "\n" << i + 1 << ". Journey Number: " << journey->journeynumber << endl;
	...
	void displayJourneysByRailwayStation(const string& railstationID) {
		...
		cout << "\n--- Journeys for Railway Station: " << railwaystation->name << " ---" << endl;
		cout << "-----------------------------" << endl;
		if (railwaystation->journeys.isEmpty()) {
		...
		while (temp) {
			Journey* journey = (Journey*)temp->data;
			cout << "\n" << count++ << ". Journey Number: " << journey->journeynumber << endl;
				...
				if (existingKeys[i] != id) {
					...
					if (existingStation) {
						double dist = MathUtils::calculateDistance(
							lat, lon,
							existingStation->latitude, existingStation->longitude
						);
						if (dist > 0 && dist < 50.0) {
							cityGraph->addUndirectedEdge(id, existingStation->id, dist);
						}
					}
				}
			}
	...
	void findNearestRailwayStation(double userLat, double userLon) {
		if (!cityGraph) {
			cout << "City graph not initialized!" << endl;
			return;
		}
		...
		cout << "\nNearest Railway Station: " << nearestRailwayStationID << endl;
